...
def single_poison_attack(
    ...
    if one_hot:
        target_mask = list(np.where(np.equal(torch.argmax(targets, dim = 1), attack_target)))
    else:
        target_mask = list(np.where(np.equal(targets, attack_target)))

    #Poison subsampling
    np.random.shuffle(target_mask[0])
    target_mask[0] = target_mask[0][:int(len(target_mask[0])*poison_prop)]
...
def write_data(
    ...
    if not os.path.isdir(os.path.join(
        save_root, 
        f"{dataset_name}_data_samples_sub{int(1/subsample_size)}x/")):
